...
export default function VideoDetailPage() {
  ...
  const { data } = useQuery({ queryKey: ['video', videoId], queryFn: () => API.getVideo(videoId) });

  const mSave = useMutation({
    mutationFn: (patch: Partial<Pick<API.VideoItem, 'title' | 'description'>>) => API.updateVideo(videoId, patch),
    onSuccess: () => {
  ...
  return (
    <Stack p="md">
      <Group justify="space-between">
        ...
        <Group>
          <Button color="red" variant="light" onClick={() => mDelete.mutate()} loading={mDelete.isPending}>
            Delete
          </Button>
          <Button
            onClick={async () => {
          ...
          <Stack>
            {data.variants.map((v) => (
              <Card key={v.variantId} withBorder>
                <Group justify="space-between">
                  <Stack gap={2}>
                    <Text fw={600}>{v.format.toUpperCase()} · {v.resolution}</Text>
                    <Text size="sm" c="dimmed">Status: {v.transcode_status}</Text>
                    {typeof v.size === 'number' && (
                      <Text size="xs" c="dimmed">Size: {v.size.toLocaleString()} bytes</Text>
                    )}
                  </Stack>
                  {v.url ? (
