...
import { Button, FileInput, Group, Modal, Progress, Stack, Text } from '@mantine/core';
...
export default function UploadDialog({
  ...
  async function doUpload() {
    ...
    try {
      ...
      if (presign.strategy === 'single') {
        await fetch(presign.url, { method: 'PUT', headers: { 'content-type': 'video/mp4' }, body: file });
        await API.completeUpload({ videoId: presign.videoId, key: presign.key });
      } else {
        const partSize = presign.partSizeBytes;
        const sent: Array<{ partNumber: number; eTag: string }> = [];
        let uploaded = 0;
        for (const part of presign.parts) {
          ...
          if (!r.ok) throw new Error(`Part ${part.partNumber} failed: ${r.status}`);
  ...
  return (
    <Modal opened={opened} onClose={onClose} title="Upload .mp4">
      <Stack>
        <FileInput accept="video/mp4" label="Choose file" value={file} onChange={setFile} />
